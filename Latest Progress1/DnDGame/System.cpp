...
Player::Player(PLAYER_ATTRIBUTES m_ePlayerAttributes):
	m_ePlayerAttributes(m_ePlayerAttributes)
{
	BeginCharacterCreation();
}
...
{
	m_ePlayerAttributes.m_nHealth -= Damage;
}

int Player::GetHealth() const
{
	return m_ePlayerAttributes.m_nHealth;
}
...
void System::HandleMonsterTurn(Monster* vMonsterInCombat)
{
	std::cout << "Monster attacks player...\n";
	std::this_thread::sleep_for(std::chrono::milliseconds(500));

	int nHitRoll = Helper::RollD(20);
	int nPlayerHitScore = 10; // Calculate this based on armor + modifiers etc

	std::cout << "Rolling a Hit Roll...\n";
	std::this_thread::sleep_for(std::chrono::seconds(2));
	std::cout << "Monster has rolled hit roll of : " + std::to_string(nHitRoll) + "\n";

	std::this_thread::sleep_for(std::chrono::milliseconds(500));
	if (nHitRoll > nPlayerHitScore)
	{
		std::cout << "Success.Rolling a damage roll...\n";
		std::this_thread::sleep_for(std::chrono::seconds(2));

		int nDmgRoll = Helper::RollD(20);
		std::cout << "Monster has Rolled Damage Roll of : " + std::to_string(nDmgRoll) + "\n";

		int nPlayerArmorModifer = 5; // Calculate this based on armor + modifiers etc

		int nDamage = nDmgRoll - nPlayerArmorModifer;
		m_pPlayer->TakeDamage(nDamage);
...
void System::HandlePlayerTurn(Monster* vMonsterInCombat)
{
	std::cout << "Player attacks Monster\n";
	...
	std::cout << "Rolling a Hit Roll...\n";
	std::this_thread::sleep_for(std::chrono::seconds(2));
	std::cout << "Player has rolled hit roll of : " + std::to_string(nHitRoll) + "\n";
	...
	if (nHitRoll > nPlayerHitScore)
	{
		...
		int nDmgRoll = Helper::RollD(20);
		std::cout << "Player has Rolled Damage Roll of : " + std::to_string(nDmgRoll) + "\n";

		int nPlayerArmorModifer = 5; // Calculate this based on armor + modifiers etc

		int nDamage = nDmgRoll - nPlayerArmorModifer;
		vMonsterInCombat->TakeDamage(nDamage);
		if (vMonsterInCombat->GetHealth() < 0)
		{
	...
	{
		std::cout << "Failed to Hit Monster...\n";
	}
...
bool System::BeginCombat(Monster* vMonsterInCombat)
{
	...
	for (int i = 0; i < vMonsterInCombat->GetPackAttributes()->size() + 1; ++i)
	{
		vInitiative.emplace_back((INITIATIVE_HELPER)i, Helper::RollD(20));
	}

	// Start combat
	while (m_pPlayer->GetHealth() > 0  && /*vMonsterInCombat->IsAnyOfPackAlive() */ vMonsterInCombat->GetHealth() > 0) // For now I will assume there's only 1 monster. Later we need to check if any of hte monters is alive, not just the main on
	{
		for (auto& object : vInitiative)
		{
			if (object.m_eCombatActor == INITIATIVE_HELPER_PLAYER_0)
				HandlePlayerTurn(vMonsterInCombat);
			else
				HandleMonsterTurn(vMonsterInCombat);
		}
	}
...
ERROR_CODE CommandDispacher::StartCombat(const std::string& strCommand)
{
	...
	*/
	...
	for (int i = 0; i < vMonstersInPoint->size(); ++i)
	{
		...
		{
			m_system.BeginCombat((*vMonstersInPoint)[i].get());
			return ERROR_CODE_CONTINUE_GAME_LOOP;
		}
