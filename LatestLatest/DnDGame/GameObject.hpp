...
/* ********************************* Object *********************************
Object is a non-moving object such as items, and various objects in the world
*/
...
public:
	...
	virtual int GetRangeOfLongRangeAttack() const = 0;

	virtual void Damage(ATTACK_INFO& eOutAttackInfo);
	virtual int GetObjectId() const;
	virtual MONSTER_ATTRIBUTES GetMonsterAttributes() const;
	virtual MONSTER_TYPE GetMonsterType() const;
	virtual int GetMovementRollThreshold() const;
	virtual int GetSpeed() const;
	virtual int SetSpeed(int nMovement);
	virtual int ResetMovement();
	virtual std::vector<MONSTER_ATTRIBUTES>* GetPackAttributes();
	virtual void TakeDamage(ATTACK_INFO& eOutAttackInfo);
	virtual int GetHealth() const;


	virtual bool IsPartOfAPack() const;
	virtual LOCATION GetLocation() const;
	virtual void SetLocation(LOCATION sMonsterLocation);
	virtual void SetVisibilityThreshold(int nVisibilityThreshold);
	virtual void InsertMonsterToPack(MONSTER_ATTRIBUTES sMonsterAttr);
	virtual bool GetIsInRangeToAttack() const;
	virtual bool GetHasLongRangeAttack() const;
	virtual int GetAC() const;
	virtual int GetConstitutionModifier() const;

protected:
	virtual void ShortRangeDamage(ATTACK_INFO& eOutAttackInfo) = 0;
	virtual void ReachRangeDamage(ATTACK_INFO& eOutAttackInfo) = 0;
	virtual void LongRangeDamage(ATTACK_INFO& eOutAttackInfo) = 0;
...
{
public:
	Wolf(MONSTER_ATTRIBUTES sMonsterAttributes, LOCATION sMonsterLocation);


private:
	void ShortRangeDamage(ATTACK_INFO& eOutAttackInfo) ;
	void ReachRangeDamage(ATTACK_INFO& eOutAttackInfo) { return; }
	void LongRangeDamage(ATTACK_INFO& eOutAttackInfo) { return; }
	int GetRangeOfLongRangeAttack() const { return 0; }
	void Bite(ATTACK_INFO& eOutAttackInfo); // 1d6 + STR (piercing)

	/*
	* To Add: Additional bonuses feature
	* During combat, before and after a monster's attack, check if the conditions
	* for their special bonuses apply.
	* 
	* The monster interface will have some functions such as CheckBonusApply
	* It will be diffeerent for each class, for example, Wolf will implement it as:
	* 
	* CheckBonusApply:
	*	- Check distance to all allies of Wolf
	*	- If anyone is less than 5 feet:
	*			- Add advantage
	* 
	* 
	* 
	Pack Tactics. 
	The wolf has Advantage on attack rolls against a creature if at least one of the wolf's allies 
...
{
public:
	Weapon(WEAPON_TYPE eWeaponType, int nRangeInSquares, bool bIsTwoHanded = false);
	virtual ~Weapon() = default;
	virtual WEAPON_TYPE GetWeaponType() const = 0;
	virtual void Damage(ATTACK_INFO& eOutAttackInfo) = 0;

protected:
	WEAPON_TYPE m_eWeaponType; // Simple, Ranged, Martial
	int m_nRangeInSquares; // Num of combat grid squares(Close-handed melee - 1 tile, Reach melee - 2 tile, and varies for ranged weapons)
	bool m_bIsTwoHanded;
};
...
{
public:
	ShortSword(WEAPON_TYPE eWeaponType = WEAPON_TYPE_SIMPLE_MELEE, int nRangeInSquares = 1);
	WEAPON_TYPE GetWeaponType() const;
	void Damage(ATTACK_INFO& eOutAttackInfo);
};
...
{
public:
	Greataxe(WEAPON_TYPE eWeaponType = WEAPON_TYPE_MARTIAL_MELEE, int nRangeInSquares = 1);
	WEAPON_TYPE GetWeaponType() const;
	void Damage(ATTACK_INFO& eOutAttackInfo);
...
public:
	...
	//std::unique_ptr<Monster>* GetMonstersInPoint(LOCATION eLocation);
	Monster* GetMonsterInPoint(const LOCATION& loc);
	bool RemoveMonster(const LOCATION& loc);

private:
	std::unique_ptr<SpawnSystem> m_pSpawnSystem;
	std::unordered_map<WorldIndex, std::unique_ptr< Monster>> m_mapMonsterLocator;
...
public:
	...
	std::unique_ptr<Wolf> CreateMonster(MONSTER_ATTRIBUTES sMonsterAttributes, LOCATION sMonsterLocation);
	std::unique_ptr<Wolf> CreateWolf(MONSTER_ATTRIBUTES sMonsterAttributes, LOCATION sMonsterLocation);
...
private:
	Factory< MONSTER_TYPE, Monster, MONSTER_ATTRIBUTES, LOCATION> m_monsterFactory;
