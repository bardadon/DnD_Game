	...
	*/
	case FEATURE_TYPE_RAGE:
	{
		pDispatcher->RegisterCommand("rage",
			[this, pDispatcher](const UserCommand& cmd, OutParameters* sOutParams)
			{
...
double Player::GetTotalPower() const
{
	return (m_ePlayerAttributes.m_sStats.m_nStrength + m_ePlayerAttributes.m_sStats.m_nDextirity +
		m_ePlayerAttributes.m_sStats.m_nConstitution + m_ePlayerAttributes.m_sStats.m_nIntelligence +
		m_ePlayerAttributes.m_sStats.m_nWisdom + m_ePlayerAttributes.m_sStats.m_nCharisma) / 6;
}

PLAYER_ATTRIBUTES Player::GetPlayerAttributes() const
{
	return m_ePlayerAttributes;
}
...
void Player::SetDefaultBarbarianStats()
{
	m_ePlayerAttributes.m_sSkills.m_nVision = 10;
	m_ePlayerAttributes.m_sSkills.m_nVisibility = 5;

	// Add All other stats and skills ..... //
}
...
bool System::PlayerChangeLocation(DIRECTION eDirection, int nNumOfSteps)
{
	...
	if (!Helper::PlayerChangeLocation(eDirection, nNumOfSteps, sCurrentLocation))
	{
		std::cout << "You can't go that way.\n";
		return false;
	}

	// Player cannot move into the same point a monster is currently at
	Monster* pMonster = m_pObjectManager->GetMonsterInPoint(sCurrentLocation);
	if (pMonster)
	{
		std::cout << "You can't go that way. There's already a monster\n";
		return false;
	}
...
void System::GameLoop()
{
	...
	//StartRenderThread();
	m_renderThread = std::thread(&System::RenderPipeThread, this);

	while (bIsPlaying)
	{
		RenderFrame frame;
		frame.width = G_ROOM_LENGTH;
		frame.height = G_ROOM_LENGTH;
		...
		// Player
		LOCATION sPlayerLocation = m_pPlayer->GetCurrentLocation();
		int px, py;
		Helper::ConvertPointIndexToXYCord(m_pPlayer->GetCurrentLocation().m_nPointIndex, px, py);
		frame.entities.push_back({ px, py, 'P' });
		Helper::ConvertPointIndexToXYCord(sPlayerLocation.m_nPointIndex, XPoint, YPoint);

		// Iterate over all points in the room
		Room* pCurrentRoom = m_pMap->GetRoom(sPlayerLocation.m_eAreaName, sPlayerLocation.m_nRoomIndex);
		for (int point = 0; point < pCurrentRoom->GetNumberOfPoints(); ++point)
		{
			// Grab all monsters in current point
			LOCATION pointLocation = sPlayerLocation;
			...
			int mx, my;
			Helper::ConvertPointIndexToXYCord(pMonsterInPoint->GetLocation().m_nPointIndex, mx, my);
			frame.entities.push_back({ mx, my, 'W' });
		}


		// Points / terrain if needed
		SendFrameToRenderer(frame);

		// Grab command from user and process it
		ERROR_CODE eErr(ERROR_CODE_GET_NEXT_COMMAND);
		while (eErr != ERROR_CODE_CONTINUE_GAME_LOOP)
		{
			std::cout << "\n[Location: " << Helper::GetAreaNameAsString(sPlayerLocation.m_eAreaName)
				<< " | RoomIdx: " << sPlayerLocation.m_nRoomIndex << " "
				<< " | PointIdx: " << sPlayerLocation.m_nPointIndex << " "
				<< " | Cords: [" << XPoint << "," << YPoint << "]> ";


			std::string strUserCommand;
			OutParameters sOutParams;
...
{
	int rollForSingleMonster = std::rand() % (G_MAX_ROOM_SIZE / G_AVERAGE_NUMBER_OF_MONSTERS_IN_ROOM);
...
{
	std::cout << "System::PopulateArea()::Populating Area\n";
	...
	for (int room = 0; room < pArea->GetNumberOfRooms(); ++room)
	{
		...
		for (int point = 1; point < pRoom->GetNumberOfPoints(); ++point) // Monster cant populate at the same point the player will be spawn at(i,e index = 0)
		{
			Point* pPoint = pRoom->GetPoint(point);
			if (!pPoint)
				continue;
...
void System::HandlePlayerTurn(Monster* pMonsterInCombat, TurnContext& sTurnContext)
{
	...
	while (bPlayersTurn)
	{
		...
		if (!sTurnContext.bUsedAction)
		{
			if (!sTurnContext.bUsedBonusAction)
				std::cout << "You spot an opportunity to act. What do you do?\n";
			else
				std::cout << "The fight slows for a heartbeat. You still have a chance to act. What do you do?\n";
		}
		else if (!sTurnContext.bUsedBonusAction)
		{
		...
		while (eErr == ERROR_CODE_GET_NEXT_COMMAND)
		{
			...
			if (sOutParams.sAttackInfo.eActionType == ACTION_TYPE_ACTION && sTurnContext.bUsedAction)
			{
				sOutParams.Clear();
				sOutParams.sTurnContext = sTurnContext;
			...
			else if (sOutParams.sAttackInfo.eActionType == ACTION_TYPE_BONUS_ACTION && sTurnContext.bUsedBonusAction)
			{
				sOutParams.Clear();
				sOutParams.sTurnContext = sTurnContext;
...
void System::PopulatePlayerVisibleMonsters()
{
	...
	for (int point = 0; point < pCurrentRoom->GetNumberOfPoints(); ++point)
	{
		...
		int nDistanceToMonster = Helper::CalculateDistance(pointLocation, m_pPlayer->GetCurrentLocation());
		double nPlayerVisionFactor = Helper::CalculateVisionFactor(m_pPlayer->GetPlayerAttributes().m_sSkills.m_nVision,
			pMonsterInPoint->GetMonsterAttributes().m_sSkills.m_nVisibility, nDistanceToMonster, pCurrentRoom->GetPoint(point)->GetPointVisibility());

		if (nPlayerVisionFactor >= pMonsterInPoint->GetMonsterAttributes().m_sSkills.m_nVisibility)
...
bool System::BeginCombat(Monster* pMonsterInCombat, TurnContext& sPlayerTurnContext)
{
	if (!pMonsterInCombat)
		return false;
	...
	std::sort(vInitiative.begin(), vInitiative.end(),
		[](const CombatActors& sCombatActor1, const CombatActors& sCombatActor2) { return sCombatActor1.m_nRollResult > sCombatActor2.m_nRollResult; });
...
void System::MoveAllMonsters()
{
	...
	*/
	...
	for (const auto& pair : m_pObjectManager->m_mapMonsterLocator)
	{
		const auto& monster = pair.second;
		if (!monster || monster.get()->GetLocation().m_eAreaName >= AREA_NAME_LAST)
			continue;

	

		DIRECTION eNewDirection;
		
		// If monster can see the player. It will behave according to the aggresive/cowardly logic
		// If not, it will move around compeletly random without the player affecting it
		
		// Check if monster can see player
		int nDistanceToPlayer = Helper::CalculateDistance(monster->GetLocation(), m_pPlayer->GetCurrentLocation());
		Room* pCurrentRoom = m_pMap->GetRoom(monster->GetLocation().m_eAreaName, monster->GetLocation().m_nRoomIndex);
		if (!pCurrentRoom)
		{
			std::cout << "System::MoveAllMonsters()::Something went wrong. Room does not exist\n";
			return;
		}

		double nMonsterVisionFactor = Helper::CalculateVisionFactor(monster->GetMonsterAttributes().m_sSkills.m_nVision,
			m_pPlayer->GetPlayerAttributes().m_sSkills.m_nVisibility, nDistanceToPlayer, pCurrentRoom->GetPoint(monster->GetLocation().m_nPointIndex)->GetPointVisibility());

		/*
		Wrap these two cases in different functions
		*/
		if (nMonsterVisionFactor < m_pPlayer->GetPlayerAttributes().m_sSkills.m_nVisibility)
		{
			// If monster is aggressive enough compared to the player's power, it will move towards the player and try to attack him
			// If the monster is not aggresive but is cowardly, it will move the opposite direction of the player and try to run away from him
			// If the monster is not aggresive and is not cowardly, it will stay put
			double dPlayerTotalPower = m_pPlayer->GetTotalPower();
			double dMonsterAggressionToPlayerPowerFactor = Helper::CalcMonsterAggressionToPlayerPowerFactor(dPlayerTotalPower, monster->GetMonsterAttributes().m_nAggression);

			// Decide on Direction and chance to move(aggresive or cowardly monsters are more likely to move)
			
			if (dMonsterAggressionToPlayerPowerFactor <= -0.4)
			{
				// Monster is very cowardly → run away from player
				eNewDirection = Helper::GetOppositeDirection(m_pPlayer->GetCurrentLocation(),
					monster->GetLocation());

				// check if the monster moves this turn
				int nMovementRoll = (std::rand() % monster->GetMovementRollThreshold()) + 1;
				if (nMovementRoll > 2)
					continue;

				std::cout << Helper::ConvertMonsterTypeToString(monster->GetMonsterType()) + " has noticed you and started to run away!\n";
			}
			else if (dMonsterAggressionToPlayerPowerFactor >= 0.4)
			{
				// Monster is very aggressive → chase player
				eNewDirection = Helper::GetDirection(m_pPlayer->GetCurrentLocation(),
					monster->GetLocation());
				...
				std::cout << Helper::ConvertMonsterTypeToString(monster->GetMonsterType()) + " has noticed you and started to towards you!\n";
			}
			else
			{
				// Neutral → move randomly or stay still
				eNewDirection = static_cast<DIRECTION>(std::rand() % DIRECTION_LAST);
		...
		// Monster moves around randomly without seeing the player
		else
		{
			// Neutral → move randomly or stay still
			eNewDirection = static_cast<DIRECTION>(std::rand() % DIRECTION_LAST);
		...
		if (!Helper::MonsterChangeLocation(eNewDirection, nNumOfSteps, m_pPlayer->GetCurrentLocation(), newLocation))
			continue;

		size_t newWorldIndex = Helper::ConvertLocationToWorldIndex(newLocation);

		// Check if target location is occupied
		const auto& MonsterAtNewLocation = m_pObjectManager->m_mapMonsterLocator[newWorldIndex];
	...
	for (const auto& move : vecPendingMoves)
	{
		...
		auto it = m_pObjectManager->m_mapMonsterLocator.find(fromIndex);
		if (it == m_pObjectManager->m_mapMonsterLocator.end())
			continue;

		auto monster = std::move(it->second);
		m_pObjectManager->m_mapMonsterLocator.erase(it);

		monster->SetLocation(
			Helper::ConvertWorldIndexToLocation(toIndex)
		);

		m_pObjectManager->m_mapMonsterLocator[toIndex] = std::move(monster);
	}
}
...
ERROR_CODE CommandDispacher::Rage(const std::string& strCommand, OutParameters* sOutParams)
{
...
// Walk multiple steps. Currently not supported
//ERROR_CODE CommandDispacher::Walk(const std::string& strCommand, DIRECTION eDirection)
//{
//	int nNumOfSteps = 1;
//
...
ERROR_CODE System::CheckBeginCombat(const std::string& strCommand, OutParameters* sOutParams)
{

	// In the future I want to allow player to attack monsters that are not in combat with him
	// If player is in combat, check if monster is already in combat with him. 
	//	- if not, add it to the combat(i,e to the initative vector)
	if (GetIsPlayerInCombat())
		return ERROR_CODE_CONTINUE_GAME_LOOP;

	// Grab monster location from strCommand(expected format is 1, 2)
	LOCATION ePlayerLocation = GetPlayerLocation();
	LOCATION monsterLocation(ePlayerLocation);

	int x = 0, y = 0;
	std::string command = strCommand;
	bool bParsingFirstNumber = true;
	bool bHasFirstNumber = false;
	bool bHasSecondNumber = false;
	bool bWasDelimiterFound = false;
	size_t pos = 0;

	// Remove user command until the first digit
	while (pos < command.size() &&
		!std::isdigit(static_cast<unsigned char>(command[pos])))
	{
		++pos;
	}
	command.erase(0, pos);
	...
	for (char c : command)
	{
		...
		{
			// comma must appear once and only after first number
			if (!bHasFirstNumber || bWasDelimiterFound)
			{
				std::cout << "Location input is wrong. Insert a location with the following format: 'x,y' \n";
				std::this_thread::sleep_for(std::chrono::seconds(2));
				return ERROR_CODE_GET_NEXT_COMMAND;
			}

			bWasDelimiterFound = true;
			bParsingFirstNumber = false;
		}
		else
		{
			std::cout << "Location input is wrong. Insert a location with the following format: 'x,y' \n";
	...
	// final validation
	if (!(bHasFirstNumber && bHasSecondNumber && bWasDelimiterFound))
	{
		std::cout << "Location input is wrong. Insert a location with the following format: 'x,y' \n";
		std::this_thread::sleep_for(std::chrono::seconds(2));
		return ERROR_CODE_GET_NEXT_COMMAND;
	}
	Helper::ConvertXYCordToPointIndex(x, y, monsterLocation.m_nPointIndex); // Update monster point index from x,y

	// Check if attack reaches the target
	int nDistanceToMonster = Helper::CalculateDistance(monsterLocation, ePlayerLocation);
	if (nDistanceToMonster > sOutParams->sAttackInfo.nAttackDistance)
	{
		std::cout << "You cant reach that far!\n";
		std::this_thread::sleep_for(std::chrono::seconds(2));
		return ERROR_CODE_GET_NEXT_COMMAND;
	}

	// Grab monster pointer
	Monster* pMonster = m_pObjectManager->GetMonsterInPoint(monsterLocation);
	if (!pMonster)
	{
	...
	// Apply damage and start combat
	pMonster->TakeDamage(sOutParams->sAttackInfo);
	sOutParams->sTurnContext.bUsedAction = true;

	if (!BeginCombat(pMonster, sOutParams->sTurnContext))
...
{
	// Grab MainHandAttack info
	m_system.MainHandAttack(sOutParams->sAttackInfo);


	// Check if attack has begun combat
	ERROR_CODE eErr(ERROR_CODE_FAILED);
	eErr = m_system.CheckBeginCombat(strCommand, sOutParams);
...
//
//	/*
//	Grab monster location from strCommand
//	Get the monster pointer
//
//
//	Check if monster is in visible monsters
//	if it is, begin combat
//	
//	*/
//	
//
