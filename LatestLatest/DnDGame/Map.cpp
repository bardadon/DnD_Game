...
Point::Point(LOCATION sPointLocation, const std::string& strPointDescription, bool bHasVisited, 
	POINT_TYPE ePointType, bool bCanMonsterEnter, POINT_VISIBILITY ePointVisibility) :
	m_sPointLocation(sPointLocation), m_strPointDescription(strPointDescription), m_bHasVisited(bHasVisited),
	m_ePointType(ePointType), m_bCanMonsterEnter(bCanMonsterEnter), m_ePointVisibility(ePointVisibility)
{
	////////////////////// Temporary //////////////////////
...
POINT_VISIBILITY Point::GetPointVisibility() const
{
	return m_ePointVisibility;
}
...
/*	for (int area = 0; area < AREA_NAME_LAST; ++area)
	...
	m_mapAreas[AREA_NAME_EMBERLIGHT_CITADEL] = std::make_unique<Area>(AREA_NAME_EMBERLIGHT_CITADEL);
}
...
{
	Area* pArea = GetArea(eAreaName);
	if (!pArea)
		return nullptr;

	return pArea->GetRoom(nRoomIndex);
}
