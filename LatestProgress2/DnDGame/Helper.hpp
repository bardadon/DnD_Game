...
constexpr int G_STARTING_GOLD = 20;
constexpr int G_STARTING_INVENTORY_CAPACITY = 50;

constexpr int G_ROOM_LENGTH = 100;
...
constexpr int G_MAX_CLASS_LEVEL = 20;
...
struct TurnContext
{
	bool bUsedAction = false;
	bool bUsedBonusAction = false;
	bool bUsedReaction = false;

	bool bHasAttackedThisTurn = false;
	bool bIsRaging = false;

	int nAttacksMade = 0;      // for Extra Attack or multiattack
	int nBonusDamage = 0;      // rage bonus, divine smite, sneak attack etc.

	// Optional: store last action result (hit/miss/damage)
	int nLastHitRoll = -1;
	int nLastDamageDealt = 0;

	// Optional: action state machine
	ACTION_TYPE eCurrentAction = ACTION_TYPE_UNKNOWN;
...
public:
	...
	static LOCATION ConvertWorldIndexToLocation(size_t nWorldIndex)
	{
		...
		const size_t areaSize = G_ROOM_LENGTH * G_ROOM_LENGTH;
		const size_t roomSize = G_AREA_LENGTH * G_AREA_LENGTH;

		// Extract Area
		loc.m_eAreaName = static_cast<AREA_NAME>(nWorldIndex / areaSize);
		nWorldIndex %= areaSize;

		// Extract Room
		loc.m_nRoomIndex = static_cast<int>(nWorldIndex / roomSize);
		nWorldIndex %= roomSize;
